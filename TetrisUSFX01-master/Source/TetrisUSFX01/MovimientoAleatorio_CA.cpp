...
void UMovimientoAleatorio_CA::TickComponent(float DeltaTime, ELevelTick TickType, FActorComponentTickFunction* ThisTickFunction)
{
    ...
    if ((TiempoAcumulado >= TiempoLimite) && (TiempoAcumulado <= TiempoFinal)) {
        TiempoLimite += 1.0f;
        ...
        if (Padre)
        {
            ...
            auto NuevaPosicion = Padre->GetActorLocation() +
                FVector
                ( FMath::RandRange(-1,1) * AnchoMovimiento,
                    FMath::RandRange(-1, 1) * AltoMovimiento,
                    FMath::RandRange(-1, 1) * ProfundidadMovimiento);
