...
void APiece::SpawnBlocks()
{
    ...
    if (Index > 7)
    {
        const int Posibilidad = FMath::RandRange(0, 10);
        if (Posibilidad < 3)
        {
            for (auto&& YZ : YZs)
            {
                ...
                Aleatorio = NewObject<UAdaptadorMovimientoAleatorio_CA>(B);
                Aleatorio->RegisterComponent();
                Blocks.Add(B);
